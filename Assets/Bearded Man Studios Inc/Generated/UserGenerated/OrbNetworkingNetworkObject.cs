...
{
	[GeneratedInterpol("{\"inter\":[0,0.1,0.1,0]")]
	public partial class OrbNetworkingNetworkObject : NetworkObject
	{
		...
		public event FieldEvent<Vector2> SizeChanged;
		public InterpolateVector2 SizeInterpolation = new InterpolateVector2() { LerpT = 0f, Enabled = false };
		public Vector2 Size
		{
			get { return _Size; }
			set
		...
		private void RunChange_Size(ulong timestep)
		{
			if (SizeChanged != null) SizeChanged(_Size, timestep);
			if (fieldAltered != null) fieldAltered("Size", _Size, timestep);
		}
		...
		{
			SizeInterpolation.current = SizeInterpolation.target;
		...
		{
			UnityObjectMapper.Instance.MapBytes(data, _Size);
		...
		{
			_Size = UnityObjectMapper.Instance.Map<Vector2>(payload);
			SizeInterpolation.current = _Size;
			SizeInterpolation.target = _Size;
			RunChange_Size(timestep);
		...
		protected override BMSByte SerializeDirtyFields()
		{
			...
			if ((0x1 & _dirtyFields[0]) != 0)
				UnityObjectMapper.Instance.MapBytes(dirtyFieldsData, _Size);
		...
		protected override void ReadDirtyFields(BMSByte data, ulong timestep)
		{
			...
			{
				if (SizeInterpolation.Enabled)
				{
					SizeInterpolation.target = UnityObjectMapper.Instance.Map<Vector2>(data);
					SizeInterpolation.Timestep = timestep;
				}
				else
				{
					_Size = UnityObjectMapper.Instance.Map<Vector2>(data);
					RunChange_Size(timestep);
				}
			}
		...
		public override void InterpolateUpdate()
		{
			...
			if (SizeInterpolation.Enabled && !SizeInterpolation.current.UnityNear(SizeInterpolation.target, 0.0015f))
			{
				_Size = (Vector2)SizeInterpolation.Interpolate();
				//RunChange_Size(SizeInterpolation.Timestep);
			}
		...
		public OrbNetworkingNetworkObject() : base() { Initialize(); }
		public OrbNetworkingNetworkObject(NetWorker networker, INetworkBehavior networkBehavior = null, int createCode = 0, byte[] metadata = null) : base(networker, networkBehavior, createCode, metadata) { Initialize(); }
		public OrbNetworkingNetworkObject(NetWorker networker, uint serverId, FrameStream frame) : base(networker, serverId, frame) { Initialize(); }
